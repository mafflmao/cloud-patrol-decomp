...
public class SpriteText : MonoBehaviour, IUseCamera
{
	...
	public bool tagColorMultipliesWithDefaultColor;
	...
	protected virtual void Init()
	{
		...
		{
			Debug.LogWarning("Text on GameObject \"" + base.name + "\" has not been assigned either a texture or a material.", base.gameObject);
		}
		if (font != null)
		{
	...
	protected void ProcessString(string str)
	{
		...
		int num4 = -1;
		int num5 = -1;
		float num6 = ((!maxWidthInPixels) ? maxWidth : (maxWidth * worldUnitsPerScreenPixel));
		text = str;
		...
		if (parseColorTags)
		{
			...
			do
			{
				num3 = str.IndexOf(colDel[0], num3);
				if (num3 != -1)
				{
					colorTags.Add(num3);
					num3++;
			...
			do
			{
				num3 = str.IndexOf(colDel[1], num3);
				if (num3 != -1)
				{
					colorTags.Add(num3);
					num3++;
		...
		if (colorTags.Count < 1)
		{
			...
			{
				num6 = ((!maxWidthInPixels) ? maxWidth : (maxWidth * worldUnitsPerScreenPixel));
				for (int i = 0; i < str.Length; i++)
				{
					...
					num6 = ((i == num5 + 1 || i <= 0) ? (num6 - spriteFont.GetAdvance(str[i]) * worldUnitsPerTexel * characterSpacing) : (num6 - spriteFont.GetWidth(str[i - 1], str[i]) * worldUnitsPerTexel * characterSpacing));
					if (!(num6 < 0f))
					{
						continue;
					}
					num6 = ((i != num4) ? (((!maxWidthInPixels) ? maxWidth : (maxWidth * worldUnitsPerScreenPixel)) - spriteFont.GetWidth(displaySB, num4 + 1, i) * worldUnitsPerTexel * characterSpacing) : ((!maxWidthInPixels) ? maxWidth : (maxWidth * worldUnitsPerScreenPixel)));
					if (num6 < 0f)
					{
						if (displaySB.Length > 0)
						{
							displaySB.Insert(displaySB.Length - 1, '\n');
							num6 = ((!maxWidthInPixels) ? maxWidth : (maxWidth * worldUnitsPerScreenPixel)) - spriteFont.GetAdvance(str[i]) * worldUnitsPerTexel * characterSpacing;
					...
					{
						displaySB[num4] = '\n';
						newLineInserts.Add(new NewlineInsertInfo(num5, 0));
					}
				}
		...
		{
			if (num == colorTags[num3])
			{
				if (string.Compare(str, num, colDel[0], 0, colDel[0].Length) == 0)
				{
					num += colDel[0].Length;
					num2 = str.IndexOf(')', colorTags[num3]) - num;
					if (num2 < 0)
					{
						num2 = str.Length - num;
					}
					colorInserts.Add(displaySB.Length);
					cols.Add(ParseColor(str.Substring(num, num2)));
					num += num2 + 1;
				}
				else if (string.Compare(str, num, colDel[1], 0, colDel[1].Length) == 0)
				{
					...
					colorInserts.Add(displaySB.Length);
					cols.Add(ParseHexColor(str.Substring(num, num2)));
					num += num2 + 1;
				}
				num3++;
				continue;
			...
			if (maxWidth > 0f && multiline)
			{
				...
				num6 = ((num == num5 + 1 || num <= 0) ? (num6 - spriteFont.GetAdvance(str[num]) * worldUnitsPerTexel * characterSpacing) : (num6 - spriteFont.GetWidth(str[num - 1], str[num]) * worldUnitsPerTexel * characterSpacing));
				if (num6 < 0f)
				{
					num6 = ((num != num5) ? (((!maxWidthInPixels) ? maxWidth : (maxWidth * worldUnitsPerScreenPixel)) - spriteFont.GetWidth(displaySB, num4 + 1, num) * worldUnitsPerTexel * characterSpacing) : ((!maxWidthInPixels) ? maxWidth : (maxWidth * worldUnitsPerScreenPixel)));
					if (num6 < 0f)
					{
						if (displaySB.Length > 0)
						{
							displaySB.Insert(displaySB.Length - 1, '\n');
							num6 = ((!maxWidthInPixels) ? maxWidth : (maxWidth * worldUnitsPerScreenPixel)) - spriteFont.GetAdvance(str[num]) * worldUnitsPerTexel * characterSpacing;
			...
			num2 = ((colorTags[num3] != -1) ? (colorTags[num3] - num) : (str.Length - num));
			plainSB.Append(str, num, num2);
			if (password)
			{
	...
	protected Color ParseColor(string str)
	{
		...
		if (tagColorMultipliesWithDefaultColor)
		{
	...
	public virtual void SetCharacterSize(float size)
	{
		if (spriteFont != null)
		{
			pixelPerfect = false;
