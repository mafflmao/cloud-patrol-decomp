...
public class UITextField : AutoSpriteControlBase, IKeyFocusable
{
	...
	public string methodOnFocus = string.Empty;
	...
	public override void OnInput(ref POINTER_INFO ptr)
	{
		...
		RaycastHit hitInfo;
		if (hasFocus && ptr.evt == POINTER_INFO.INPUT_EVENT.PRESS && caret != null && allowClickCaretPlacement && base.GetComponent<Collider>().Raycast(ptr.ray, out hitInfo, ptr.rayDepth))
		{
			PositionInsertionPoint(hitInfo.point);
		}
	...
	{
		if (scriptWithMethodToInvoke != null && !string.IsNullOrEmpty(methodOnFocus))
		{
			scriptWithMethodToInvoke.Invoke(methodOnFocus, 0f);
		}
		if (commitDelegate != null)
		{
			commitDelegate(this);
	...
	protected void PositionCaret(bool recur)
	{
		...
		if (recur)
		{
			...
			else
			{
				...
				{
					Vector3 centerPoint = GetCenterPoint();
					Vector3 localPosition = spriteText.transform.localPosition + Vector3.right * (Mathf.Abs(centerPoint.x - vector.x) * 1.9f);
					localPosition.x = Mathf.Min(localPosition.x, origTextPos.x);
					spriteText.transform.localPosition = localPosition;
					PositionCaret(false);
					spriteText.ClippingRect = clientClippingRect;
				...
				{
					Vector3 localPosition2 = spriteText.transform.localPosition - Vector3.right * caretSize.y;
					spriteText.transform.localPosition = localPosition2;
					PositionCaret(false);
					spriteText.ClippingRect = clientClippingRect;
					return;
