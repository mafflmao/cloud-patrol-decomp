...
public abstract class AutoSpriteControlBase : AutoSpriteBase, IEZDragDrop, IControl, IPackableControl, IUIObject, ISpritePackable
{
	...
	public bool GetColliderFromStyleSheet;
	...
	public virtual void UpdateCollider()
	{
		...
		boxCollider.isTrigger = true;
		if (GetColliderFromStyleSheet)
		{
			Vector3 colliderComponent = GetComponent<StyleSheet>().colliderComponent;
			Vector3 size = boxCollider.size;
			size.x *= colliderComponent.x;
			size.y *= colliderComponent.y;
			size.z *= colliderComponent.z;
			boxCollider.size = size;
			boxCollider.center += GetComponent<StyleSheet>().colliderOffset;
		}
	}
	...
	public virtual void FindOuterEdges()
	{
		...
		for (int i = 0; i < aggregateLayers.Length; i++)
		{
			for (int j = 0; j < aggregateLayers[i].Length; j++)
			{
				if (!aggregateLayers[i][j].IsHidden() && aggregateLayers[i][j].gameObject.activeSelf)
				{
