...
public abstract class AutoSpriteBase : SpriteBase, ISpriteAggregator, ISpritePackable
{
	...
	{
		base.Awake();
		if (m_spriteMesh != null && ((SpriteMesh)m_spriteMesh).MRAtlasIndex != -1)
		{
			for (int i = 0; i < States.Length; i++)
			{
				TextureAnim textureAnim = States[i];
				Vector2[] array = new Vector2[4];
				for (int j = 0; j < textureAnim.frameGUIDs.Length; j++)
				{
					textureAnim.spriteFrames[j].uvs.x = array[0].x;
					textureAnim.spriteFrames[j].uvs.y = array[1].y;
					textureAnim.spriteFrames[j].uvs.width = array[2].x - array[1].x;
					textureAnim.spriteFrames[j].uvs.height = array[0].y - array[1].y;
				}
			}
		}
		...
		{
			animations[k] = new UVAnimation();
			animations[k].SetAnim(States[k], k);
		}
	}
	...
	{
		if (deleted || !base.gameObject.activeInHierarchy)
		{
			return;
		}
		if (!m_started)
	...
	{
		if (!animating && Application.isPlaying && base.gameObject.activeInHierarchy)
		{
			animating = true;
