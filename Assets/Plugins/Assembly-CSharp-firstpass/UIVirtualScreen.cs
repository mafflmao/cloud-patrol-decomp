...
public class UIVirtualScreen : MonoBehaviour, IEZDragDrop, IUIObject
{
    ...
    {
        if (!(screenCamera == null) && !screenCamera.gameObject.activeSelf)
        {
    ...
    public IUIObject GetControl(ref POINTER_INFO ptr)
    {
        ...
        bool flag = !screenCamera.gameObject.activeSelf;
        if (flag)
        {
            screenCamera.gameObject.SetActive(true);
        }
    ...
    // Explicit interface implementations should be implemented as properties
