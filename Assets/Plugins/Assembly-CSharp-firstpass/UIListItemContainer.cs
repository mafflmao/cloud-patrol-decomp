...
public class UIListItemContainer : ControlBase, IUIListObject, IEZDragDrop, IUIContainer, IUIObject
{
	...
	public void FindOuterEdges()
	{
		...
		if (spriteText != null)
		{
			...
			zero = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(spriteText.UnclippedTopLeft));
			zero2 = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(spriteText.UnclippedBottomRight));
			topLeftEdge.x = Mathf.Min(topLeftEdge.x, zero.x);
			topLeftEdge.y = Mathf.Max(topLeftEdge.y, zero.y);
			bottomRightEdge.x = Mathf.Max(bottomRightEdge.x, zero2.x);
			bottomRightEdge.y = Mathf.Min(bottomRightEdge.y, zero2.y);
		}
		for (int i = 0; i < textObjs.Count; i++)
		{
			textObjs[i].Start();
			Matrix4x4 localToWorldMatrix = textObjs[i].transform.localToWorldMatrix;
			zero = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(textObjs[i].UnclippedTopLeft));
			zero2 = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(textObjs[i].UnclippedBottomRight));
			topLeftEdge.x = Mathf.Min(topLeftEdge.x, zero.x);
			topLeftEdge.y = Mathf.Max(topLeftEdge.y, zero.y);
			bottomRightEdge.x = Mathf.Max(bottomRightEdge.x, zero2.x);
			bottomRightEdge.y = Mathf.Min(bottomRightEdge.y, zero2.y);
		...
		for (int j = 0; j < uiObjs.Count; j++)
		{
			...
			if (uiObjs[j] is AutoSpriteControlBase)
			{
				...
				zero = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(((AutoSpriteControlBase)uiObjs[j]).TopLeftEdge));
				zero2 = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(((AutoSpriteControlBase)uiObjs[j]).BottomRightEdge));
			}
			else
			{
				zero = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(uiObjs[j].UnclippedTopLeft));
				zero2 = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(uiObjs[j].UnclippedBottomRight));
			}
			topLeftEdge.x = Mathf.Min(topLeftEdge.x, zero.x);
			topLeftEdge.y = Mathf.Max(topLeftEdge.y, zero.y);
			bottomRightEdge.x = Mathf.Max(bottomRightEdge.x, zero2.x);
