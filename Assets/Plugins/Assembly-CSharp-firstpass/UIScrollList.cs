...
public class UIScrollList : MonoBehaviour, IEZDragDrop, IUIObject
{
	...
	public float OffsetCenterScrollList;

	protected Vector3 OffsetMover;
	...
	public bool m_ScrollToSelected = true;
	...
	public Callback methodToInvokeOnSnap;

	public SpriteManager manager;

	public bool detargetOnDisable;

	public IUIObject ClicMaster;
	...
	protected IUIListObject focusedItem;

	protected bool focusChange;

	protected int lastFocusIndex;

	public CallbackReturn<GameObject, bool, bool> cycleItem;

	public bool isCycle;

	public bool isDynamic;

	public bool isPoolScroll;

	private int m_indexfocus;

	private int m_NbPanelToDisplay;

	private int m_MaxPanelInstantiate;

	private CallbackReturn<GameObject, int> m_CallbackGetItem;
	...
	protected Callback beginDragDelegate;

	protected Callback endMoveDelegate;
	...
	public bool m_ScaleFocusedItem;

	public float m_ScaleFactor = 1.5f;

	public float m_ScaleCenterProximity = 5f;
	...
	protected float scrollStopThresholdLog = Mathf.Log10(0.0001f);
	...
	protected bool AutoScrolling
	{
		...
		set
		{
			...
			if (flag)
			{
				if (!isScrolling && !autoScrolling && beginDragDelegate != null)
				{
					beginDragDelegate();
				}
			}
			else if (!isScrolling && autoScrolling && endMoveDelegate != null)
			{
				endMoveDelegate();
			}
			autoScrolling = flag;
	...
	protected void Awake()
	{
		...
		try
		{
			Transform transform = base.transform.Find("Mover");
			if (transform == null)
			{
				flag = true;
			}
	...
	public void SetupScroll(int i_IndexStart, int i_NbPanelRoDisplay, int i_MaxPanelInstantiate, CallbackReturn<GameObject, int> i_Callback, float i_timeToScroll)
	{
		m_indexfocus = i_IndexStart;
		m_NbPanelToDisplay = i_NbPanelRoDisplay;
		m_MaxPanelInstantiate = i_MaxPanelInstantiate;
		m_CallbackGetItem = i_Callback;
		InitScrollist(i_timeToScroll);
	}

	private void InitScrollist(float i_timeToScroll)
	{
		if (m_indexfocus <= m_NbPanelToDisplay)
		{
			for (int i = 0; i < m_NbPanelToDisplay; i++)
			{
				AddItem(m_CallbackGetItem(i));
			}
		}
		else
		{
			for (int j = m_indexfocus - m_NbPanelToDisplay / 2; j < m_indexfocus + m_NbPanelToDisplay / 2; j++)
			{
				AddItem(m_CallbackGetItem(j));
			}
		}
		ScrollToItem(m_indexfocus, i_timeToScroll);
	}

	public void UpdatePanelDynamic(int i_PanelNumber)
	{
		if (i_PanelNumber >= m_indexfocus - m_NbPanelToDisplay / 2 && i_PanelNumber <= m_indexfocus + m_NbPanelToDisplay / 2)
		{
			ReplaceItem(i_PanelNumber - (m_indexfocus - m_NbPanelToDisplay / 2), m_CallbackGetItem(i_PanelNumber), false, false);
		}
	}
	...
	protected virtual void ScrollListTo_Internal(float pos)
	{
		if (!float.IsNaN(pos) && !(mover == null))
		{
			...
			OffsetMover.x = mover.transform.localPosition.x + OffsetCenterScrollList / 100f * viewableArea.x;
			OffsetMover.y = mover.transform.localPosition.y;
			OffsetMover.z = mover.transform.localPosition.z;
			mover.transform.localPosition = OffsetMover;
			scrollPos = pos;
			ClipItems();
			if (slider != null)
			{
	...
	{
		snappedItem = item;
		if (methodToInvokeOnSnap != null)
		{
			methodToInvokeOnSnap();
		}
		if (focusedItem == null)
		{
			focusedItem = item;
	...
	public void InsertItem(IUIListObject item, int position, string text, bool doEasing)
	{
		...
		if (activateWhenAdding && !((Component)item).gameObject.activeSelf)
		{
		...
		{
			item.Hide(true);
			if (!item.Managed && !(item is IUIContainer))
			{
	...
	{
		if (mover == null || items.Count < 1 || !clipContents || !base.gameObject.activeSelf)
		{
			return;
		}
		IUIListObject iUIListObject = null;
		...
		if (orientation == ORIENTATION.HORIZONTAL)
		{
			...
			if (m_ScaleFocusedItem)
			{
				for (int j = 0; j < items.Count; j++)
				{
					float num3 = Mathf.Abs(items[j].transform.localPosition.x + x - OffsetCenterScrollList / 100f * viewableArea.x);
					if (num3 < m_ScaleCenterProximity)
					{
						items[j].transform.localScale = Vector3.one * Mathf.Lerp(m_ScaleFactor, 1f, num3 / m_ScaleCenterProximity);
					}
					else
					{
						items[j].transform.localScale = Vector3.one;
					}
				}
			}
			if (direction == DIRECTION.TtoB_LtoR)
			{
				...
				if (iUIListObject != null)
				{
					...
					if (!iUIListObject.gameObject.activeSelf)
					{
		...
		else
		{
			...
			float num5 = viewableAreaActual.y * -0.5f - y;
			int k = (int)((float)(items.Count - 1) * Mathf.Clamp01(scrollPos));
			if (m_ScaleFocusedItem)
			{
				for (int l = 0; l < items.Count; l++)
				{
					float num6 = Mathf.Abs(items[l].transform.localPosition.y + y - OffsetCenterScrollList / 100f * viewableArea.y);
					if (num6 < m_ScaleCenterProximity)
					{
						items[l].transform.localScale = Vector3.one * Mathf.Lerp(m_ScaleFactor, 1f, num6 / m_ScaleCenterProximity);
					}
					else
					{
						items[l].transform.localScale = Vector3.one;
					}
				}
			}
			if (direction == DIRECTION.TtoB_LtoR)
			{
				float y2 = items[k].transform.localPosition.y;
				if (items[k].BottomRightEdge.y + y2 <= num4)
				{
					for (k--; k > -1; k--)
					{
						y2 = items[k].transform.localPosition.y;
						if (items[k].BottomRightEdge.y + y2 > num4)
						{
							break;
						}
					}
					iUIListObject = items[k + 1];
				}
				else
				{
					for (; k < items.Count; k++)
				...
				if (iUIListObject != null)
				{
					...
					if (iUIListObject.BottomRightEdge.y + y2 > num5)
					{
						for (k = iUIListObject.Index + 1; k < items.Count; k++)
						{
							y2 = items[k].transform.localPosition.y;
							if (items[k].BottomRightEdge.y + y2 <= num5)
							{
								...
								items[k].Hide(false);
								items[k].ClippingRect = clientClippingRect;
								tempVisItems.Add(items[k]);
								break;
							}
							if (!items[k].gameObject.activeSelf)
							{
			...
			{
				float y2 = items[k].transform.localPosition.y;
				if (items[k].TopLeftEdge.y + y2 >= num5)
				{
					for (k--; k > -1; k--)
					{
						y2 = items[k].transform.localPosition.y;
				...
				if (iUIListObject != null)
				{
					...
					iUIListObject.ClippingRect = clientClippingRect;
					y2 = iUIListObject.transform.localPosition.y;
					if (iUIListObject.TopLeftEdge.y + y2 < num4)
					{
						for (k = iUIListObject.Index + 1; k < items.Count; k++)
						{
							y2 = items[k].transform.localPosition.y;
							if (items[k].TopLeftEdge.y + y2 >= num4)
							{
								if (!items[k].gameObject.activeSelf)
								{
									items[k].gameObject.SetActive(true);
		...
		if (visibleItems.Count > 0)
		{
			...
			{
				for (int m = 0; m < visibleItems.Count; m++)
				{
					visibleItems[m].Hide(true);
					if (!visibleItems[m].Managed && !(visibleItems[m] is IUIContainer))
					{
						visibleItems[m].gameObject.SetActive(false);
					}
				}
			}
			else
			{
				for (int n = 0; n < visibleItems.Count && visibleItems[n].Index < iUIListObject.Index; n++)
				{
					visibleItems[n].Hide(true);
					if (!visibleItems[n].Managed && !(visibleItems[n] is IUIContainer))
					{
						visibleItems[n].gameObject.SetActive(false);
					}
				}
				int num7 = visibleItems.Count - 1;
				while (num7 > -1 && visibleItems[num7].Index > iUIListObject2.Index)
				{
					visibleItems[num7].Hide(true);
					if (!visibleItems[num7].Managed && !(visibleItems[num7] is IUIContainer))
					{
						visibleItems[num7].gameObject.SetActive(false);
					}
					num7--;
				}
			}
	...
	public void DidSelect(IUIListObject item, POINTER_INFO ptr)
	{
		SelectedItem = item;
		if (m_ScrollToSelected)
		{
			ScrollToItem(item, minSnapDuration);
		}
	}

	public void DidClick(IUIObject item, POINTER_INFO ptr)
	{
		lastClickedControl = item;
		if (item is IActionButton)
		{
			IActionButton actionButton = (IActionButton)item;
			actionButton.InvokeAllMethodForEvent(ptr);
		}
		else if (scriptWithMethodToInvoke != null)
		{
		...
		if (m_ScrollToSelected)
		{
			ScrollToItem((IUIListObject)item, minSnapDuration);
		}
	}

	public void ListDragged(POINTER_INFO ptr)
	{
		if ((ClicMaster != null && ClicMaster != ptr.targetObj) || !touchScroll || !controlIsEnabled)
		{
			return;
		}
		AutoScrolling = false;
		...
		noTouch = false;
		IsScrolling = true;
		UIManager.instance.Retarget(ptr.targetObj, this);
	}
	...
	public void RemoveItem(int index, bool destroy, bool doEasing)
	{
		if (index >= 0 && index < items.Count)
		{
			...
			isCycle = false;
			isDynamic = false;
			if (focusedItem == items[index])
			{
				focusedItem = items[Mathf.Min(index + 1, items.Count - 1)];
			}
			if (snappedItem == items[index])
			{
				snappedItem = items[Mathf.Min(index + 1, items.Count - 1)];
			}
	...
	public void ReplaceItem(int vIndex, GameObject newItem, bool destroy, bool doEasing)
	{
		ReplaceItem(vIndex, newItem.GetComponent<UIListItemContainer>(), destroy, doEasing);
	}

	public void ReplaceItem(int vIndex, IUIListObject newItem, bool destroy, bool doEasing)
	{
		bool flag = isCycle;
		bool flag2 = isDynamic;
		isCycle = false;
		isDynamic = false;
		if (focusedItem == items[vIndex])
		{
			focusedItem = newItem;
		}
		if (snappedItem == items[vIndex])
		{
			snappedItem = newItem;
		}
		RemoveItem(vIndex, destroy, doEasing);
		InsertItem(newItem, vIndex, doEasing);
		isCycle = flag;
		isDynamic = flag2;
	}
	...
	public virtual void LateUpdate()
	{
		...
		{
			float num = 0f;
			num = ((orientation != 0) ? Mathf.Clamp01((focusedItem.TopLeftEdge.y - focusedItem.BottomRightEdge.y + itemSpacing) / amtOfPlay) : Mathf.Clamp01((focusedItem.BottomRightEdge.x - focusedItem.TopLeftEdge.x + itemSpacing) / amtOfPlay));
			int num2 = m_NbPanelToDisplay / 2;
			if (lastFocusIndex < focusedItem.Index)
			{
				if (m_MaxPanelInstantiate % 2 == 1)
				{
					m_indexfocus++;
				}
				...
				if (isDynamic)
				{
					if (focusedItem.Index > num2 && m_indexfocus + num2 <= m_MaxPanelInstantiate && Count < m_MaxPanelInstantiate)
					{
						if (m_indexfocus < 0)
						{
							m_indexfocus = 0;
						}
						gameObject = m_CallbackGetItem(m_indexfocus + num2);
					}
				}
				else if (cycleItem != null)
				{
					gameObject = cycleItem(true, flag);
				}
				else
				{
					gameObject = items[0].gameObject;
					flag = false;
				}
				if (gameObject != null)
				{
					RemoveItem(items[0], flag, false);
					AddItem(gameObject);
					ScrollListTo_Internal(scrollPos - num);
					if (AutoScrolling)
					{
						ScrollToItem(snappedItem, autoScrollDuration - autoScrollTime);
					}
				}
				if (m_MaxPanelInstantiate % 2 == 0)
				{
					m_indexfocus++;
				}
			}
			else
			{
				...
				if (isDynamic)
				{
					if (focusedItem.Index < num2 && m_indexfocus - num2 > 0)
					{
						if (m_indexfocus > m_MaxPanelInstantiate)
						{
							m_indexfocus = m_MaxPanelInstantiate - num2 - 1;
						}
						gameObject2 = m_CallbackGetItem(m_indexfocus - num2 - 1);
					}
				}
				else if (cycleItem != null)
				{
					gameObject2 = cycleItem(false, flag2);
				}
				else
				{
					gameObject2 = items[items.Count - 1].gameObject;
					flag2 = false;
				}
				if (gameObject2 != null)
				{
					RemoveItem(items[items.Count - 1], flag2, false);
					IUIListObject component = gameObject2.GetComponent<UIListItemContainer>();
					InsertItem(component, 0);
					ScrollListTo_Internal(scrollPos + num);
					if (AutoScrolling)
					{
						ScrollToItem(snappedItem, autoScrollDuration - autoScrollTime);
	...
	private void CycleList(float i_Pos)
	{
		if (Count < 1 || focusedItem == null || focusChange)
		{
			return;
		}
		float num = 0f;
		...
		if (orientation == ORIENTATION.HORIZONTAL)
		{
			num = Mathf.Clamp01((focusedItem.BottomRightEdge.x - focusedItem.TopLeftEdge.x + itemSpacingActual) / amtOfPlay);
			num2 = Mathf.Clamp01(focusedItem.transform.localPosition.x / amtOfPlay);
			if (i_Pos > num2 + num)
			{
				if (isDynamic && focusedItem.Index + 1 < items.Count)
				{
					lastFocusIndex = focusedItem.Index;
					focusChange = true;
					focusedItem = items[focusedItem.Index + 1];
				}
			}
			else if (i_Pos < num2 - num && isDynamic && focusedItem.Index - 1 >= 0)
			{
				lastFocusIndex = focusedItem.Index;
				focusChange = true;
				focusedItem = items[focusedItem.Index - 1];
		...
		num = Mathf.Clamp01((focusedItem.TopLeftEdge.y - focusedItem.BottomRightEdge.y + itemSpacingActual) / amtOfPlay);
		num2 = Mathf.Clamp01(Mathf.Abs(focusedItem.transform.localPosition.y) / amtOfPlay);
		float num3 = num2 + num;
		float num4 = num2 - num;
		if (i_Pos > num3)
		{
		...
		else if (i_Pos < num4 && isDynamic && focusedItem.Index - 1 >= 0)
		{
			lastFocusIndex = focusedItem.Index;
			focusChange = true;
			focusedItem = items[focusedItem.Index - 1];
	...
	private void DynamicList(float i_Pos)
	{
		if (Count < 1 || focusedItem == null || focusChange)
		{
			return;
		...
		if (orientation == ORIENTATION.HORIZONTAL)
		{
			...
			else if (i_Pos < num2 - num + num * 0.15f && isDynamic && focusedItem.Index - 1 >= 0)
			{
				lastFocusIndex = focusedItem.Index;
				focusChange = true;
				focusedItem = items[focusedItem.Index - 1];
		...
		num = Mathf.Clamp01((focusedItem.TopLeftEdge.y - focusedItem.BottomRightEdge.y + itemSpacingActual) / amtOfPlay);
		num2 = Mathf.Clamp01(Mathf.Abs(focusedItem.transform.localPosition.y) / amtOfPlay);
		float num3 = num2 + num * 1.5f;
		float num4 = num2 - num * 1.5f;
		if (i_Pos > num3)
		{
			if (isDynamic && focusedItem.Index + 1 < items.Count)
			{
	...
	public void AddOnBeginDragDelegate(Callback del)
	{
		beginDragDelegate = (Callback)Delegate.Combine(beginDragDelegate, del);
	}

	public void RemoveOnBeginDragDelegate(Callback del)
	{
		beginDragDelegate = (Callback)Delegate.Remove(beginDragDelegate, del);
	}

	public void AddOnEndMoveDelegate(Callback del)
	{
		endMoveDelegate = (Callback)Delegate.Combine(endMoveDelegate, del);
	}

	public void RemoveOnEndMoveDelegate(Callback del)
	{
		endMoveDelegate = (Callback)Delegate.Remove(endMoveDelegate, del);
	}
